...
const PokeInfo: FC = () => {
  const [info, setInfo] = useState<PokeData | null>(null);
  useEffect(() => {
    const getInfo = async () => {
      const response = await getPokeInfo();
  ...
  return (
    <>
      <div className="w-[500px] divide-y divide-gray-100 rounded-md border border-gray-200 px-2.5">
        ...
        <div className="mt-6 border-t border-gray-100">
          <dl className="divide-y divide-gray-100">
            <div className="px-4 py-6 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-0">
              ...
              <dd className="mt-1 text-sm leading-6 sm:col-span-2 sm:mt-0">Margot Foster</dd>
            </div>
            <div className="px-4 py-6 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-0">
              <dt className="text-sm font-medium leading-6">Application for</dt>
              <dd className="mt-1 text-sm leading-6 sm:col-span-2 sm:mt-0">Backend Developer</dd>
            </div>
            <div className="px-4 py-6 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-0">
...
const getPokeInfo = async () => {
  try {
    const id = 1;
    const response: AxiosResponse<PokeData, any> = await api.get(`pokemon/${id}/`);
    return response;
  } catch (error) {
